...
export const getPageNumbers = ({ currentPage, pageSize, total, pageNumbersToShow = 3 }: GetPageNumbersArgs) => {
  ...
  if (typeof last === "number" && last !== lastPageNumber) {
    if (last === lastPageNumber - 1) {
      pageNumbers = [...pageNumbers, lastPageNumber];
    } else {
      const ellipsis = last < lastPageNumber - 2 ? "..." : lastPageNumber - 1;
